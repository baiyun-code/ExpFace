...
# parser.add_argument('--config', default="casia_magnitude", help='the name of config file')
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        resnet = get_model(network, dropout=0, fp16=False, num_features=cfg.embedding_size).cuda()
...
def get_image_feature(img_path, files_list, model_path, epoch, gpu_id):
    ...
    faceness_scores = []
    batch = 0
    img_feats = np.empty((len(files), cfg.embedding_size*2), dtype=np.float32)
