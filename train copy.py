...
from partial_fc_v2 import PartialFC_V2, my_PFC, LoraFC, my_CE,my_BCE,my_BCE1,my_CE_logexp1,my_CE_logexp,my_CE_logexp2
...
def main(config_file):
    ...
    margin_loss = PowerFace(s=64,m=0.7)
    # margin_loss=ArcFace_s(32, 0.5, Forward_s)
    # margin_loss=PowerFace_norm1(s=cfg.s,m=0.55)
    ...
    if cfg.resume:
        dict_checkpoint = torch.load(os.path.join(cfg.output, f"checkpoint_gpu_21.pt"),weights_only=True)
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        for i, (img, local_labels) in enumerate(train_loader):
            ...
            if cfg.save_all_states and (global_step in [1,10,100]) :
                checkpoint = {
                "epoch": epoch + 1,
                ...
                torch.save(checkpoint, os.path.join(cfg.output, f"checkpoint_gpu_it_{global_step}.pt"))
                del checkpoint
